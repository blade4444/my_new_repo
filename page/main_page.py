...
class MainPage:
    ...
    el_hover_change_xp = "class name=nav_sol_par user_prof_menu hovered"
    log_out_button_xp = "class name=h_user_logout icon-font-logout"
    ...
    # iframe_for_into_the_window = "//iframe[@id='webWidget']"
    ...
    def verify_working_logo_button(self):
        ...
        try:
            self.driver_h.waiting_for_an_item_to_appear(self.video_play_button_xp)
        except Exception as err:
            raise Exception("###########################################################################\n"
                            "Error: After click logo button, page do not changed\n")
    ...
    def open_support_window(self):
        ...
        # self.driver_h.operation_into_iframe(iframe_butt=self.in_iframe_button_xp, message=data.name, el_for_message=self.input_name_xp,
        #                                     element=self.supp_butt_in_right_bottom, iframe_for_mess=self.iframe_for_into_the_window)

    def write_message_to_support(self):
        ...
        self.driver_h.filling_in_a_field(element=self.input_email_xp, message_text=data.email)
        self.driver_h.filling_in_a_field(element=self.input_message_xp, message_text=data.message)

    def send_mesage_to_support(self):
